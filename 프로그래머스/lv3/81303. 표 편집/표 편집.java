...
모든 X들의 합이 100만 이하라고 한게 힌트이다. LinkedList를 직접 구현하라는 의도 같다.
역순으로 StringBuilder insert로 효율성 초과가 나지 않는 이유이다. 어짜피 100만 이하의 이동을 하기 때문이다.

[LinkedList 직접 구현]
-> 배열이나 리스트에 노드들을 순차적으로 넣어놓기
-> 나중에 제거된 노드인지 확인해서 O,X를 한 번에 더해줄 수 있다.
*/